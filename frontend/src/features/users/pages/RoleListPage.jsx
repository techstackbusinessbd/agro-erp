    ...
    <div className="pb-10 min-h-screen space-y-8">
      ...
      <div className="sticky top-4 z-20 flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl p-6 rounded-md border border-gray-100/50 dark:border-gray-800/50 shadow-xl shadow-gray-500/5 transition-all mb-8">
          <div className="space-y-1 flex-shrink-0">
              ...
              <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Access Roles</h1>
              <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest leading-none">Security Permissions</p>
          </div>
          ...
          <div className="flex items-center gap-6 flex-shrink-0">
              <div className="flex gap-6 pr-6 border-r border-gray-100 dark:border-gray-800">
                  <div className="text-right">
                      <p className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">Total Roles</p>
                      <p className="text-2xl font-black text-gray-900 dark:text-white leading-none mt-1">{roles.length}</p>
                  </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white dark:bg-gray-900 rounded-md p-6 border border-gray-100 dark:border-gray-800 animate-pulse">
              <div className="flex items-center justify-between mb-6">
                <div className="w-10 h-10 bg-gray-100 dark:bg-gray-800 rounded"></div>
                <div className="w-20 h-4 bg-gray-100 dark:bg-gray-800 rounded"></div>
              </div>
              <div className="h-6 bg-gray-100 dark:bg-gray-800 rounded w-3/4 mb-4"></div>
              <div className="space-y-2">
                <div className="h-3 bg-gray-50 dark:bg-gray-800 rounded w-full"></div>
                <div className="h-3 bg-gray-50 dark:bg-gray-800 rounded w-2/3"></div>
              </div>
            </div>
          ))
        ) : filteredRoles.length > 0 ? (
          filteredRoles.map((role) => (
            <div key={role.id} className="group/card relative bg-white dark:bg-gray-900 rounded-md border border-gray-100 dark:border-gray-800 overflow-hidden hover:shadow-2xl hover:shadow-primary-500/5 transition-all duration-300">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                ...
                <h3 className="text-lg font-black text-gray-800 dark:text-white uppercase tracking-tight mb-2">{role.name}</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {(role.permissions || []).slice(0, 4).map((perm) => (
                    <span key={perm.id} className="px-2 py-1 bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 text-gray-500 dark:text-gray-400 text-[9px] font-black rounded uppercase tracking-widest truncate max-w-[120px]">
                      {perm.name}
                    </span>
                  ))}
          ...
          <div className="col-span-full py-24 text-center bg-white dark:bg-gray-900 rounded-md border border-gray-100 dark:border-gray-800 shadow-sm">
            ...
            <h4 className="text-lg font-black text-gray-400 uppercase tracking-widest">No Roles Discovered</h4>
            <p className="text-xs text-gray-300 dark:text-gray-600 uppercase tracking-[0.2em] mt-2">Try adjusting your filters</p>
          </div>
        )}
      </div>
